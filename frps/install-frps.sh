...
program_version_link="https://raw.githubusercontent.com/Bet5521/onekey-install-shell/master/frps/version.sh"
str_install_shell=https://raw.githubusercontent.com/Bet5521/onekey-install-shell/master/frps/install-frps.sh
shell_update(){
    fun_clangcn "clear"
...
fun_get_version(){
    ...
    if [ -z ${FRPS_VER} ] || [ -z ${FRPS_INIT} ] || [ -z ${ghproxy_download_url} ] || [ -z ${github_download_url} ]; then
...
fun_getServer(){
    ...
    case "${set_server_url}" in
        1|[Aa][Ll][Ii][Yy][Uu][Nn])
            program_download_url=${ghproxy_download_url}
            ;;
        2|[Gg][Ii][Tt][Hh][Uu][Bb])
